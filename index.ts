...
async function main() {
  ...
  const safe4337Pack = await Safe4337Pack.init({
    ...
    options: {
      owners: [nearAdapter.address],
      threshold: 1,
      saltNonce: "0",
    },
  });
  safe4337Pack.protocolKit.getAddress();
  ...
  const userOp = {
    ...
    verificationGasLimit: safeNotDeployed ? 500000n : 100000n,
  ...
  const safeOpHash = await m4337.getOperationHash({
    ...userOp,
