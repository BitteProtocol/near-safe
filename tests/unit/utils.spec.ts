...
describe("Utility Functions (mostly byte packing)", () => {
  it("PLACE_HOLDER_SIG", () => {
    expect(PLACEHOLDER_SIG).toEqual(encodePacked(["uint48", "uint48"], [0, 0]));
  });
  it("packGas", () => {
    let [hi, lo] = [15n, 255n];
    expect(packGas(hi, lo)).toEqual(
      encodePacked(["uint128", "uint128"], [hi, lo])
    );
    // Random input
    expect(packGas(15, "255")).toEqual(
      encodePacked(["uint128", "uint128"], [15n, BigInt("0xff")])
    );
  });
  it("packSignature", () => {
    const x =
    ...
    expect(packSignature(x, 1, 2)).toEqual(
      encodePacked(["uint48", "uint48", "bytes"], [1, 2, x])
    );
  });
  ...
  it("packPaymasterData", () => {
    ...
    expect(packPaymasterData(data).toLowerCase()).toEqual("");
  });
